...
class Alias(models.Model):
    ...
    def find_matches(self, alias=None, lw_iteration=None) -> list[Tenure]:
        if alias is None:
            alias = self.alias
        court_tenures = Tenure.objects.filter(court=self.court)
        matches = {}
        if lw_iteration is None:
            ...
            for tenure in court_tenures:
                name = " ".join(
                    self.standardize_name(tenure.person.name_canonical).split(" ")[-lw_iteration:]
                )
                matches[tenure] = jaro_winkler_similarity(alias, name)
        return matches
    ...
    def match_tenure(self, update=False):
        ...
        if matches[top_match] > 0.9:
            ...
            print("Matched through JW score on first attempt")
            ...
            if try_alias != self.alias:
                ...
                if matches[top_match] > 0.9:
                    # setting tenure to the top match
                    print("Matched through JW score in retry")
                    self.tenure = top_match
                    self.save()

                else:
                    alias_terms = try_alias.split(" ")
                    build_out_term = ""

                    for i, term in reversed(list(enumerate(alias_terms))):
                        build_out_term = " ".join([term, build_out_term]).strip()
                        matches = self.find_matches(term, i)
                        top_match = max(matches, key=lambda k: matches[k])
                        build_out_name = " ".join(
                            top_match.person.name_canonical.split(" ")[-i:]
                        ).strip()

                        if matches[top_match] > 0.9:
                            print("Matched through JW score in retry by iteration")
                            self.tenure = top_match
                            self.save()
                            break

                        elif build_out_name == build_out_term:
                            print(
                                "Matched through exactly matching last name in retry by iteration"
                            )
                            self.tenure = top_match
                            self.save()
                            break
