...
from .models import Court, Person, Election, Candidacy, Tenure, Case, \
                    IndividualOpinion, CourtType, SelectionType, CaseType, \
                    PartyAffiliation
...
def add_fake_data(request):
    ...
    for _ in range(10):
        Person.objects.create(
            name = fake.name(),
            birth_date = fake.date_between(start_date = "-150y", \
                                           end_date = "-22y"),
            gender = fake.passport_gender(),
            race = random.choice(["White", "Black", "Asian"]),
            party_registration = random.choice(["Republican", "Democrat", \
                                                "Independent", "Other"]),
    ...
    for court_data in courts:
        Court.objects.get_or_create(org_id=court_data["org_id"], 
                                    defaults=court_data)
    ...
    for election_data in elections:
        Election.objects.get_or_create(court=election_data["court"],
    ...
    for election in election_objects:
        ...
        for person in candidates:
            Candidacy.objects.get_or_create(person = person, 
                                            election = election)
    ...
    for _ in range(3):
        for person in persons:
            ...
            start = fake.date_between(start_date=date(1950, 1, 1), \
                                      end_date=date(2020, 1, 1))
            end = fake.date_between(start_date=start, end_date=date(2024, 1, 1))
            appointer_party = (random.choice(PartyAffiliation.values) 
            ...
            Tenure.objects.get_or_create(
                ...
                defaults={
                    ...
                    "ticket_party": (random.choice(PartyAffiliation.values) 
                        if selection == SelectionType.PARTISAN 
                        else ""),
                    "appointer_name": (fake.name() 
                        if selection == SelectionType.APPOINTMENT 
                        else ""),
    ...
    for _ in range(10):
        Case.objects.create(
            ...
            decision_outcome=random.choice(["Def good for justice",
                "This one owned the libs", "This one killed all the penguins",
